...
public class BannerPostRearAttachmentModel extends RearAttachmentRenderModel {
    public static final EntityModelLayer MODEL_LAYER = new EntityModelLayer(Motobox.id("vehicle/rear_attachment/banner_post"), "main");
    ...
    public void setRenderState(@Nullable RearAttachment attachment, float wheelAngle, float tickDelta) {
        ...
        float push = attachment == null ? 0 : (float) Math.pow(Math.max(0, attachment.vehicle().getHSpeed() * 0.368f), 2);
        ...
        if (attachment instanceof BannerPostRearAttachment bannerPost) {
            ...
            this.flag.setAngles(push, this.flag.yaw, 0.05f * (float) Math.sin((attachment.vehicle().getTime() + tickDelta) / 20));
        }
