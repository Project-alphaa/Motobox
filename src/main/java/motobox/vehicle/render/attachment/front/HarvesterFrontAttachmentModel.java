...
public class HarvesterFrontAttachmentModel extends FrontAttachmentRenderModel {
    public static final EntityModelLayer MODEL_LAYER = new EntityModelLayer(Motobox.id("vehicle/front_attachment/harvester"), "main");
    ...
    public void setRenderState(@Nullable FrontAttachment attachment, float groundHeight, float tickDelta) {
        ...
        if (this.roller != null) {
            if (attachment != null) {
                this.roller.setAngles((float) Math.toRadians(attachment.vehicle().getWheelAngle(tickDelta)), 0, 0);
            } else {
