...
public class BaseChestRearAttachment extends BlockRearAttachment {
    ...
    public BaseChestRearAttachment(RearAttachmentType<?> type, VehicleEntity entity, BlockState block, @Nullable BiFunction<ScreenHandlerContext, BlockRearAttachment, NamedScreenHandlerFactory> screenProvider) {
    ...
    public void open(PlayerEntity player) {
        if (!player.isSpectator()) {
            this.viewerManager.openContainer(player, this.world(), this.vehicle.getBlockPos(), Blocks.AIR.getDefaultState());
        }
    }

    public void close(PlayerEntity player) {
        if (!player.isSpectator()) {
            this.viewerManager.closeContainer(player, this.world(), this.vehicle.getBlockPos(), Blocks.AIR.getDefaultState());
        }
    }
    ...
    public static BaseChestRearAttachment chest(RearAttachmentType<?> type, VehicleEntity entity) {
    ...
    public static BaseChestRearAttachment enderChest(RearAttachmentType<?> type, VehicleEntity entity) {
        return new BaseChestRearAttachment(type, entity,
                ...
                (ctx, att) -> new SimpleNamedScreenHandlerFactory((syncId, inventory, player) -> {
                    ...
                    if (att instanceof BaseChestRearAttachment chest) {
                        EnderChestInventoryDuck.of(enderChest).motobox$setActiveAttachment(chest);
                    }
    ...
    public static BaseChestRearAttachment saddledBarrel(RearAttachmentType<?> type, VehicleEntity entity) {
